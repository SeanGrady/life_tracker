...
class CronometerDailySummary(Base, AppUserMixin):
    ...
    B5_Pantothenic Acid_mg = Column(Float)
    B6_Pyridoxine_mg = Column(Float)
    B12_Cobalamin_µg = Column(Float)
    Folate_µg = Column(Float)
    Vitamin A_IU = Column(Float)
    Vitamin C_mg = Column(Float)
    Vitamin D_IU = Column(Float)
    Vitamin E_mg = Column(Float)
    Vitamin K_µg = Column(Float)
